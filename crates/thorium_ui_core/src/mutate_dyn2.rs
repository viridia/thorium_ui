...
pub struct MutateDyn2Effect<P, M, EffectFn: Fn(P, &mut EntityWorldMut)> {
    deps: Option<P>,
    deps_sys: SystemId<(), P>,
    effect_fn: EffectFn,
...
impl<P: 'static + PartialEq + Send + Sync + Clone, M, EffectFn: Fn(P, &mut EntityWorldMut)>
    AnyEffect for MutateDyn2Effect<P, M, EffectFn>
{
...
impl<
        ...
        EffectFn: Fn(P, &mut EntityWorldMut) + Send + Sync + 'static,
    > MutateDyn2<P, M, DepsFn, EffectFn>
{
...
unsafe impl<
        ...
        DepsFn: IntoSystem<(), P, M> + Send + Sync + 'static,
        EffectFn: Fn(P, &mut EntityWorldMut) + Send + Sync + 'static,
    > Bundle for MutateDyn2<P, M, DepsFn, EffectFn>
{
    fn component_ids(
...
impl<
        ...
        DepsFn: IntoSystem<(), P, M> + Send + Sync + 'static,
        EffectFn: Fn(P, &mut EntityWorldMut) + Send + Sync + 'static,
    > DynamicBundle for MutateDyn2<P, M, DepsFn, EffectFn>
{
    type Effect = Self;
