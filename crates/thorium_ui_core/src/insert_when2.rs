...
pub struct InsertWhenEffect2<B: Bundle, FactoryFn: Fn() -> B> {
    state: bool,
    test_sys: SystemId<(), bool>,
    factory: FactoryFn,
}

impl<B: Bundle, FactoryFn: Fn() -> B> AnyEffect for InsertWhenEffect2<B, FactoryFn> {
...
impl<
        ...
        B: Bundle,
        FactoryFn: Fn() -> B + Send + Sync + 'static,
    > InsertWhen2<M, TestFn, B, FactoryFn>
{
...
unsafe impl<
        ...
        B: Bundle,
        FactoryFn: Fn() -> B + Send + Sync + 'static,
    > Bundle for InsertWhen2<M, TestFn, B, FactoryFn>
{
    fn component_ids(
...
impl<
        ...
        B: Bundle,
        FactoryFn: Fn() -> B + Send + Sync + 'static,
    > DynamicBundle for InsertWhen2<M, TestFn, B, FactoryFn>
{
    type Effect = Self;
